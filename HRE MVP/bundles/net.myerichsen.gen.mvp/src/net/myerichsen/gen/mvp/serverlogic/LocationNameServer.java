 ...
 * {@link net.myerichsen.gen.mvp.dbmodels.LocationNames}
 *
...
public class LocationNameServer {
	...
	public String[] getNameStrings() throws SQLException {
		...
		int locationPid = name.getLocationPid();
		...
		for (int i = 0; i < nameList.size(); i++) {
			...
			List<LocationNameParts> npl = new LocationNameParts().getFKLocationNamePid(aName.getLocationNamePid());

			// Concatenate non-null name parts
			for (LocationNameParts nameParts : npl) {
	...
	public String getPrimaryNameString(int locationPid) throws SQLException {
		...
		for (int i = 0; i < nameList.size(); i++) {
			...
			if (aName.isPrimaryLocationName()) {
				List<LocationNameParts> lnp = new LocationNameParts().getFKLocationNamePid(aName.getLocationNamePid());

				// Concatenate non-null name parts
				for (LocationNameParts nameParts : lnp) {
					if (nameParts.getLocationNamePid() == aName.getLocationNamePid()) {
