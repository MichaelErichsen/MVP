...
import net.myerichsen.gen.mvp.dbmodels.LocationNameMaps;
import net.myerichsen.gen.mvp.dbmodels.LocationNameParts;
import net.myerichsen.gen.mvp.dbmodels.LocationNameStyles;
import net.myerichsen.gen.mvp.dialogs.DateDialog;
import net.myerichsen.gen.mvp.dialogs.DateNavigatorDialog;
import net.myerichsen.gen.mvp.providers.HDateProvider;
import net.myerichsen.gen.mvp.providers.LocationNameProvider;
...
public class LocationNameView {
	...
	private LocationNameProvider provider;
	...
	public void createControls(Composite parent) {
		...
		Label lblStyle = new Label(composite_1, SWT.NONE);
		lblStyle.setText("Location Name Style");

		textLocationNameStylePid = new Text(composite_1, SWT.BORDER);
		textLocationNameStylePid.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1));
		...
		textLocationNameStylePid.setToolTipText("Double click to edit location name style");

		textLocationNameStyle = new Text(composite_1, SWT.BORDER);
		textLocationNameStyle.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1));
		textLocationNameStyle.setEditable(false);
		
				Label lblPreposition = new Label(composite_1, SWT.NONE);
				lblPreposition.setText("Preposition");
		
				textPreposition = new Text(composite_1, SWT.BORDER);
				textPreposition.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 2, 1));

		Label lblLocationNameParts = new Label(composite_1, SWT.NONE);
		lblLocationNameParts.setText("Location Name Parts");

		tableViewerNameParts = new TableViewer(composite_1, SWT.BORDER | SWT.FULL_SELECTION);
		tableNameParts = tableViewerNameParts.getTable();
		tableNameParts.setLayoutData(new GridData(SWT.LEFT, SWT.FILL, false, true, 2, 1));
		...
		TableViewerColumn tableViewerColumnId = new TableViewerColumn(tableViewerNameParts, SWT.NONE);
		TableColumn tblclmnId = tableViewerColumnId.getColumn();
		tblclmnId.setWidth(50);
		tblclmnId.setText("ID");

		TableViewerColumn tableViewerColumnLabelFromMaps = new TableViewerColumn(tableViewerNameParts, SWT.NONE);
		TableColumn tblclmnLabelFromMaps = tableViewerColumnLabelFromMaps.getColumn();
		tblclmnLabelFromMaps.setWidth(200);
		tblclmnLabelFromMaps.setText("Label from Maps");

		TableViewerColumn tableViewerColumnValueFromParts = new TableViewerColumn(tableViewerNameParts, SWT.NONE);
		TableColumn tblclmnValueFromParts = tableViewerColumnValueFromParts.getColumn();
	...
	private void get(int locationNamePid) {
		try {
			...
			textLocationNameStylePid.setText(Integer.toString(provider.getLocationNameStylePid()));
			...
			try {
				int toDatePid = provider.getFromDatePid();
				hdp.get(toDatePid);
				textToDate.setText(hdp.getDate().toString());
				textToOriginal.setText(hdp.getOriginalText());
			} catch (Exception e) {
			...
			List<LocationNameParts> partList = provider.getPartList();
			List<LocationNameMaps> mapList = provider.getMapList();

			for (int i = 0; i < partList.size(); i++) {
				TableItem item = new TableItem(tableNameParts, SWT.NONE);
				item.setText(0, Integer.toString(partList.get(i).getLocationNamePartPid()));
				item.setText(1, mapList.get(i).getLabel());
				item.setText(2, partList.get(i).getLabel());
			}

			eventBroker.post("MESSAGE", "Location Name " + textId.getText() + " has been fetched");
		} catch (Exception e) {
			clear();
			eventBroker.post("MESSAGE", e.getMessage());
			LOGGER.severe(e.getMessage());
	...
	protected void insert() {
		try {
			...
			provider.setLocationNameStylePid(Integer.parseInt(textLocationNameStylePid.getText()));
	...
	protected void openLocationNamePartView() {
		...
		final ParameterizedCommand command = commandService
				.createCommand("net.myerichsen.gen.mvp.command.openlocationnamepartview", null);
		...
		eventBroker.post(net.myerichsen.gen.mvp.Constants.LOCATION_NAME_PART_PID_UPDATE_TOPIC, namePartPid);
	}
	...
	protected void openLocationNameStyleView() {
		...
		// Open Location Name Map View
		final ParameterizedCommand command = commandService
				.createCommand("net.myerichsen.gen.mvp.command.openlocationnamestyleview", null);
		handlerService.executeHandler(command);

		String nameStyle = textLocationNameStyle.getText();
		...
		try {
			...
			for (LocationNameStyles locationNameStyles : styleList) {
				if (locationNameStyles.getLabel().equals(nameStyle)) {
					locationNameStylePid = locationNameStyles.getLocationNameStylePid();
					break;
				}
			}
		...
		eventBroker.post(net.myerichsen.gen.mvp.Constants.LOCATION_NAME_STYLE_PID_UPDATE_TOPIC, locationNameStylePid);
	}
	...
	@Inject
	@Optional
	private void subscribeNameUpdateTopic(@UIEventTopic(Constants.LOCATION_NAME_PID_UPDATE_TOPIC) int locationNamePid)
