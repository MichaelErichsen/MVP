...
import net.myerichsen.gen.mvp.listeners.HexFocusListener;
import net.myerichsen.gen.mvp.listeners.HexVerifyListener;
import net.myerichsen.gen.mvp.listeners.IntegerListener;
import net.myerichsen.gen.mvp.listeners.LengthFocusListener;
import net.myerichsen.gen.mvp.listeners.NumericVerifyListener;
import net.myerichsen.gen.mvp.listeners.SmallIntListener;
...
public class H2TableEditor {
		...
		btnInsert.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				try {
					...
					eventBroker.post(net.myerichsen.gen.mvp.databaseadmin.HreDataAccessConstants.TABLENAME_UPDATE_TOPIC,
							tableName);
	...
	private void createLines() throws SQLException {
		...
		for (int i = 0; i < count; i++) {
			...
			case HreDataAccessConstants.BOOLEAN:
				...
				break;
			case HreDataAccessConstants.CHAR:
				text = createFieldLine(compositeFields, i);
			...
			case HreDataAccessConstants.CLOB:
				...
				lineList.add(text);
				break;
			case HreDataAccessConstants.DATE:
				text = createFieldLine(compositeFields, i);
				columns.get(i).setValue(row.get(i));
				if (row.get(i) != null) {
					text.setText((String) row.get(i));
				...
				lineList.add(text);
				break;
			case HreDataAccessConstants.DECIMAL:
				text = createFieldLine(compositeFields, i);
				columns.get(i).setValue(row.get(i));
				if (row.get(i) != null) {
					text.setText((String) row.get(i));
	...
	private void subscribeRecordNumUpdateTopic(
			@UIEventTopic(net.myerichsen.gen.mvp.databaseadmin.HreConstants.RECORDNUM_UPDATE_TOPIC) String recordNumString)
			throws SQLException {
